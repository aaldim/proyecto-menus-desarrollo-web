...
interface ClassifierButtonProps {
  classifierId: number; // Recibimos el ID del producto como prop
}

export const ClassifierDeleteButton = ({ classifierId }: ClassifierButtonProps) => {
  const handleDelete = async () => {
    // Hacemos una solicitud para ejecutar la Server Action
    const res = await fetch(`/api/classifier/${classifierId}`, {
      method: "DELETE",
    });
